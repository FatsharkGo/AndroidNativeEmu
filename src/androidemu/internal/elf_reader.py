...
class ELFFile(elftools.elf.elffile.ELFFile):
    '''ELF file load helper
    '''
    def __init__(self, filename: str):
        self._fstream = open(filename, 'rb')
        super(ELFFile, self).__init__(self._fstream)
        if self.header.e_type != 'ET_DYN':
            raise NotImplementedError("Only ET_DYN is supported at the moment.")

    def __del__(self):
        if self._fstream is not None:
            self._fstream.close()

    def get_load(self):
        #for x in self.iter_segments():
        #    print(x.header.p_type)
        return [x for x in self.iter_segments() if x.header.p_type == 'PT_LOAD']
    ...
    def get_rels(self):
        ...
        for section in self.iter_sections():
            if not isinstance(section, elftools.elf.relocation.RelocationSection):
                continue
            rels[section.name]=list(section.iter_relocations())
        return rels

    def get_dyn_string_by_rel_sym(self, rel_sym):
        pass

    def get_init_array(self, load_base: int):
        init_array_size = 0
        init_array_offset = 0
        init_array = []
        for x in self.iter_segments():
