...
class MemoryMap:
    ...
    def __map(self, address, size, prot=UC_PROT_READ | UC_PROT_WRITE):
        ...
        try:
            ...
            #impossible
            for r in self.__mu.mem_regions():
                logger.error("region begin :0x%08X end:0x%08X, prot:%d"%(r[0], r[1], r[2]))
            #
            raise
        #
    #
    ...
    def map(self, address, size, prot=UC_PROT_READ | UC_PROT_WRITE, vf=None, offset=0):
        ...
        logger.debug("map addr:0x%08X, end:0x%08X, sz:0x%08X off=0x%08X"%(address, address+size, size, offset))
        ...
        if (res_addr != -1 and vf != None):
            ...
            logger.debug("read for offset %d sz %d data sz:%d"%(offset, size, len(data)))
            #print("data:%r"%data)
    ...
    def protect(self, addr, len, prot):
        ...
        try:
            ...
            #TODO:just for debug
            logger.error("Warning mprotect with addr: 0x%08X len: 0x%08X prot:0x%08X failed!!!"%(addr, len, prot))
